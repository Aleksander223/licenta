...
router.get("/evaluations/bins", [verifyUser, verifyAdmin], async (req, res) => {
  try {
    ...
    for (
      let i = dayOfYear(session.startDate);
      i < dayOfYear(session.endDate);
      i++
    ) {
      const now = new Date();
...
router.get("/report/:id", [verifyUser, verifyProfessor], async (req, res) => {
  try {
    ...
    const courses = evaluations
      .map((x) => (x.course as ICourse)._id)
      .filter((x, i, self) => {
        return self.indexOf(x) === i;
      });

    const types = evaluations
      .map((x) => x.type)
      .filter((x, i, self) => {
        return self.indexOf(x) === i;
      });
    ...
    for (const course of courses) {
      for (const type of types) {
        ...
        const matched = evaluations.filter(
          (x) => (x.course as ICourse)._id == course && x.type == type
        );
        ...
        for (const match of matched) {
          ...
          const text =
            match.answers[match.answers.length - 1].questions[0].choice;
